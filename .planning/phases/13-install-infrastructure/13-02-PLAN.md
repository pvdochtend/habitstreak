...
must_haves:
  truths:
    ...
    - "Dismissal state persists in localStorage (source of truth) and syncs to database (write-only backup)"
  ...
  key_links:
    - from: "src/contexts/pwa-install-context.tsx"
      to: "/api/user"
      via: "fetch for dismissal sync (write-only)"
...
4. **Dismissal persistence** (IMPORTANT - per-device, not per-user):
   - localStorage key: `habitstreak-pwa-dismissed`
   - localStorage is the SOURCE OF TRUTH for dismissal state
   - On mount: read from localStorage ONLY - do NOT fetch dismissal state from database
   - On dismiss: set localStorage immediately, then PATCH to /api/user as write-only backup
   - Database sync is for analytics/backup purposes only - never read back to override local state
   - Handle unauthenticated users gracefully (localStorage-only, skip API call)
...
- NEVER override localStorage dismissal state from database fetch - dismissal is per-device
  </action>
  <verify>
...
- Dismissal persists to localStorage immediately (source of truth)
- Dismissal syncs to database as write-only backup for logged-in users
- Database state is NEVER read back to override localStorage (per-device requirement)
...
6. Verify dismissal on Device A does NOT affect Device B (per-device requirement)
</verification>
...
- Dismissal state persists across page refreshes via localStorage (source of truth)
- Dismissal syncs to database as write-only backup (not read back)
- Dismissal is per-device: dismissing on one device does NOT dismiss on another
